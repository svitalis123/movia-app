...
export function InlineSpinner({
  ...
  return (
    <Loader2
      className={`animate-spin ${size === 'small' ? 'h-4 w-4' : 'h-6 w-6'} ${className || ''}`}
    />
  );
}
...
export function LoadingOverlay({
  ...
  return (
    <div
      className={`absolute inset-0 flex items-center justify-center bg-background/50 backdrop-blur-sm rounded-md ${className || ''}`}
    >
