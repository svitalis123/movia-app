  ...
  return (
    ...
    >
      <div className="flex items-start">
        {showIcon && (
          <IconComponent
            className={`${sizes.icon} ${styles.iconColor} mt-0.5 flex-shrink-0`}
          />
        )}

        <div className={`flex-1 ${showIcon ? 'ml-3' : ''}`}>
          {title && <h3 className={`${sizes.title} mb-1`}>{title}</h3>}
...
export function InlineErrorMessage({
  ...
  return (
    <div
      className={`flex items-center gap-1 text-destructive text-sm ${className || ''}`}
    >
