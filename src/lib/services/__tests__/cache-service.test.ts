...
describe('CacheKeyGenerator', () => {
  ...
  it('should generate correct search movies key', () => {
    expect(CacheKeyGenerator.searchMovies('test query')).toBe(
      'search_test_query_page_1'
    );
    expect(CacheKeyGenerator.searchMovies('Test Query', 2)).toBe(
      'search_test_query_page_2'
    );
    expect(CacheKeyGenerator.searchMovies('  multiple   spaces  ')).toBe(
      'search_multiple_spaces_page_1'
    );
  });
  ...
  it('should generate correct movies by genre key', () => {
    ...
    expect(CacheKeyGenerator.moviesByGenre(28, 3)).toBe(
      'genre_28_movies_page_3'
    );
  });

  it('should generate correct similar movies key', () => {
    expect(CacheKeyGenerator.similarMovies(123)).toBe(
      'similar_movies_123_page_1'
    );
    expect(CacheKeyGenerator.similarMovies(123, 2)).toBe(
      'similar_movies_123_page_2'
    );
  });

  it('should generate correct movie recommendations key', () => {
    expect(CacheKeyGenerator.movieRecommendations(456)).toBe(
      'recommendations_456_page_1'
    );
    expect(CacheKeyGenerator.movieRecommendations(456, 3)).toBe(
      'recommendations_456_page_3'
    );
  });
});
