...
test.describe('Smoke Tests', () => {
  ...
  test('should have proper meta tags for responsive design', async ({ page }) => {
    await page.goto('/');
    await page.waitForTimeout(2000);
  ...
  test('should handle routing without crashing', async ({ page }) => {
    ...
    for (const route of routes) {
      ...
      const hasBrowserError = pageContent && (
        ...
        (pageContent.includes('404 Not Found') && pageContent.includes('nginx'))
      );
  ...
  test('should handle authentication flow', async ({ page }) => {
    ...
    const isOnExpectedDomain = currentUrl.includes('localhost') || 
